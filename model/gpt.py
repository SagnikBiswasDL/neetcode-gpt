...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.TokenEmbedding = nn.Embedding(vocab_size, model_dim)
        self.PositEmbedding = nn.Embedding(context_length, model_dim)
        ...
        for i in range(num_blocks):
            layers.append(self.TransformerBlock(model_dim, num_heads))
        
        self.model = nn.Sequential(*layers)

        self.layer_norm = nn.LayerNorm(model_dim)

        self.fin = nn.Linear(model_dim, vocab_size)
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        print(self.TokenEmbedding)
        print(context)
        token_x = self.TokenEmbedding(context)
        posit_x = self.PositEmbedding(torch.arange(self.C))
